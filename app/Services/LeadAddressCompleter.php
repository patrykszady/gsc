 ...
 *
 * Ported essentially verbatim from hive2025's
 * app/Services/LeadAddressCompleter.php — this is pure policy (no
 * hive-specific dependencies beyond GeoapifyService), so nothing here
 ...
 * street is refused rather than resolved to the wrong state).
 *
 * Incomplete is a valid outcome: the lead still saves, and the admin's
 * Location column just shows whatever parts are known.
 */
class LeadAddressCompleter
{
    ...
    public function complete(array $leadData): array
    {
        ...
        if (($onlyStreet = self::streetOnly($street)) !== null) {
     ...
     *
     * public/static: also called directly by ContactSubmission::street() to
     * serve a clean `street` value on the API — that's a display concern
     * with no geocoding to do, so it reuses this parsing rather than
     * standing up a whole LeadAddressCompleter (which needs a GeoapifyService).
     */
    public static function streetOnly(string $street): ?string
    {
