<x-layouts.app
    :title="isset($area) ? 'About Us | Home Remodeling in ' . $area->city . ' | GS Construction' : 'About Us | GS Construction | Family-Owned Home Remodeling'"
...
>
    ...
    <main class="isolate">
                ...
                <div class="mx-auto max-w-7xl px-6 pt-12 pb-16 sm:pt-16 lg:px-8 lg:pt-12">
                    <div class="mx-auto max-w-2xl gap-x-14 lg:mx-0 lg:flex lg:max-w-none lg:items-center">
                        <div class="relative w-full lg:max-w-xl lg:shrink-0 xl:max-w-2xl">
                            <p class="text-sm font-semibold uppercase tracking-wide text-sky-600 dark:text-sky-400">{{ isset($area) ? 'About Us in ' . $area->city : 'About Us' }}</p>
                            <h1 class="font-heading mt-2 text-4xl font-bold tracking-tight text-zinc-900 sm:text-6xl dark:text-white">
                                {{ isset($area) ? 'Trusted Family Remodelers in ' . $area->city : 'A Family Business Built on Trust' }}
                            </h1>
                            <p class="mt-8 text-lg font-medium text-zinc-600 sm:max-w-md sm:text-xl/8 lg:max-w-none dark:text-zinc-300">
                                @if(isset($area))
                                Serving {{ $area->city }} and surrounding communities, GS Construction & Remodeling is more than a business—it's a family legacy. Run by Gregory and Patryk, a father-son duo with over 40 years of combined experience, we bring heart, skill, and dedication to every {{ $area->city }} home.
                                @else
                        ...
                        <div class="mt-14 flex justify-end gap-4 sm:-mt-44 sm:justify-start sm:pl-20 lg:mt-0 lg:pl-0">
                            <div class="ml-auto w-40 flex-none space-y-4 pt-32 sm:ml-0 sm:pt-80 lg:order-last lg:pt-36 xl:order-0 xl:pt-80">
                                ...
                                <div class="relative">
                                    <img src="{{ $galleryImages[0]->getThumbnailUrl('medium') }}" alt="{{ $galleryImages[0]->alt_text ?? 'GS Construction project' }}" class="aspect-square w-full rounded-xl bg-zinc-900/5 object-cover shadow-lg dark:bg-zinc-700/5" loading="lazy" />
                                ...
                                <div class="relative">
                                    <img src="{{ $galleryImages[5]->getThumbnailUrl('medium') }}" alt="{{ $galleryImages[5]->alt_text ?? 'GS Construction project' }}" class="aspect-square w-full rounded-xl bg-zinc-900/5 object-cover shadow-lg dark:bg-zinc-700/5" loading="lazy" />
                                    <div class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-zinc-900/10 ring-inset dark:ring-white/10"></div>
                                </div>
                                @endif
                            </div>
                            <div class="mr-auto w-40 flex-none space-y-4 sm:mr-0 sm:pt-52 lg:pt-36">
                                ...
                                <div class="relative">
                                    <img src="{{ $galleryImages[1]->getThumbnailUrl('medium') }}" alt="{{ $galleryImages[1]->alt_text ?? 'GS Construction project' }}" class="aspect-square w-full rounded-xl bg-zinc-900/5 object-cover shadow-lg dark:bg-zinc-700/5" loading="lazy" />
                                    <div class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-zinc-900/10 ring-inset dark:ring-white/10"></div>
                                </div>
                                @endif
                                @if($galleryImages->count() > 2)
                                <div class="relative">
                                    <img src="{{ $galleryImages[2]->getThumbnailUrl('medium') }}" alt="{{ $galleryImages[2]->alt_text ?? 'GS Construction project' }}" class="aspect-square w-full rounded-xl bg-zinc-900/5 object-cover shadow-lg dark:bg-zinc-700/5" loading="lazy" />
                                    <div class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-zinc-900/10 ring-inset dark:ring-white/10"></div>
                                </div>
                                @endif
                            </div>
                            <div class="w-40 flex-none space-y-4 pt-32 sm:pt-0">
                                ...
                                <div class="relative">
                                    <img src="{{ $galleryImages[3]->getThumbnailUrl('medium') }}" alt="{{ $galleryImages[3]->alt_text ?? 'GS Construction project' }}" class="aspect-square w-full rounded-xl bg-zinc-900/5 object-cover shadow-lg dark:bg-zinc-700/5" loading="lazy" />
                                    <div class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-zinc-900/10 ring-inset dark:ring-white/10"></div>
                                </div>
                                @endif
                                @if($galleryImages->count() > 4)
                                <div class="relative">
                                    <img src="{{ $galleryImages[4]->getThumbnailUrl('medium') }}" alt="{{ $galleryImages[4]->alt_text ?? 'GS Construction project' }}" class="aspect-square w-full rounded-xl bg-zinc-900/5 object-cover shadow-lg dark:bg-zinc-700/5" loading="lazy" />
                                    <div class="pointer-events-none absolute inset-0 rounded-xl ring-1 ring-zinc-900/10 ring-inset dark:ring-white/10"></div>
                                </div>
                                @endif
                            </div>
        ...
        {{-- Team section (disabled for now)
        <div class="mx-auto mt-16 max-w-7xl px-6 sm:mt-20 lg:px-8">
            <div class="mx-auto max-w-2xl lg:mx-0">
                <h2 class="font-heading text-3xl font-bold tracking-tight text-zinc-900 sm:text-4xl dark:text-white">Meet the Team</h2>
                <p class="mt-6 text-lg/8 text-zinc-600 dark:text-zinc-400">
                    The father-son duo behind every GS Construction project.
                </p>
            </div>
            <ul role="list" class="mx-auto mt-12 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-12 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-2">
                <li class="flex flex-col sm:flex-row gap-6">
                    <img src="{{ asset('images/greg-patryk.jpg') }}" alt="Gregory" class="aspect-square w-full sm:w-52 flex-none rounded-2xl object-cover" />
                    <div>
                        <h3 class="font-heading text-xl font-bold tracking-tight text-zinc-900 dark:text-white">Gregory</h3>
                        <p class="text-base/7 text-sky-600 dark:text-sky-400">Founder & Master Craftsman</p>
                        <p class="mt-4 text-base/7 text-zinc-600 dark:text-zinc-400">
                            With over 30 years in the industry, Gregory brings unmatched expertise in carpentry, tile work, and project management. His eye for detail ensures every project exceeds expectations.
                        </p>
                    </div>
                </li>
                <li class="flex flex-col sm:flex-row gap-6">
                    <img src="{{ asset('images/greg-patryk.jpg') }}" alt="Patryk" class="aspect-square w-full sm:w-52 flex-none rounded-2xl object-cover" />
                    <div>
                        <h3 class="font-heading text-xl font-bold tracking-tight text-zinc-900 dark:text-white">Patryk</h3>
                        <p class="text-base/7 text-sky-600 dark:text-sky-400">Co-Owner & Operations Lead</p>
                        <p class="mt-4 text-base/7 text-zinc-600 dark:text-zinc-400">
                            Patryk combines modern design sensibilities with traditional craftsmanship. He handles client communications, project scheduling, and ensures every detail is perfect.
                        </p>
                    </div>
                </li>
            </ul>
        ...
        <div class="relative isolate px-6 py-10 sm:py-12 lg:px-8">
            ...
            <div class="mx-auto max-w-2xl text-center">
                <h2 class="font-heading text-4xl font-bold tracking-tight text-zinc-900 sm:text-5xl dark:text-white">
                    {{ isset($area) ? 'Ready to Transform Your ' . $area->city . ' Home?' : 'Ready to Transform Your Home?' }}
                </h2>
                <p class="mx-auto mt-6 max-w-xl text-lg/8 text-zinc-600 dark:text-zinc-300">
                    Let's discuss your project. Schedule a free consultation and see why {{ isset($area) ? $area->city : 'Chicagoland' }} homeowners trust GS Construction.
                </p>
                <div class="mt-10 flex items-center justify-center gap-x-6">
                    <flux:button href="/contact" variant="primary" class="font-semibold uppercase tracking-wide" @click="trackCTA('Schedule Free Consultation', 'about_page_cta')">
