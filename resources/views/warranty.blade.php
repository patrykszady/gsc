...
>
    ...
    <div class="mx-auto max-w-7xl px-4 pt-10 sm:px-6 lg:px-8">
        ...
        <p class="speakable text-lg leading-8 text-zinc-700 dark:text-zinc-300">
            ...
            <a href="tel:+1{{ config('brand.phone_href') }}" class="font-semibold text-sky-700 hover:underline dark:text-sky-400">{{ config('brand.phone') }}</a>,
            or email
            <a href="mailto:{{ config('brand.email') }}" class="font-semibold text-sky-700 hover:underline dark:text-sky-400">{{ config('brand.email') }}</a>.
    ...
    <x-faq-section
        ...
        :collapsed="true"
            :faqs="[
                ...
                ['question' => 'How do I get something looked at?', 'answer' => 'Call or text ' . config('brand.phone') . ' or email ' . config('brand.email') . ' — whether your project finished last month or three years ago. You deal directly with Greg and Patryk, no claims department.'],
            ]"
    />
