...
/** Ported from hive2025 with StreetAddress itself. */
class StreetAddressTest extends TestCase
{
    #[DataProvider('streets')]
    public function test_knows_a_street_from_a_landmark(?string $value, bool $expected): void
    {
        $this->assertSame($expected, StreetAddress::looksLikeStreet($value));
    }
    ...
    #[DataProvider('texts')]
    public function test_reads_the_street_address_out_of_a_message(string $text, ?array $expected): void
    {
        $this->assertSame($expected, StreetAddress::findInText($text));
    }
